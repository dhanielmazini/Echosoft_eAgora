...
public class URLImageRequest {
    public static String RequestURL(String photoRef, int width, int height, Coordenada local) {
        ...
        else return "https://maps.googleapis.com/maps/api/staticmap?center=" + local.getLatitude() +
                    "," + local.getLongitude() + "&maptype=roadmap&markers=color:red%7Clabel:C%7C" +
                    local.getLatitude() + "," +
                    local.getLongitude() +
                    "&zoom=15&scale=2&size=150x60&key=AIzaSyADceanowFmdgs2F1_jnKjyeXCuRa8IrWo";
    }
}
